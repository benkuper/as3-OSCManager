package org.tuio.osc {
	...
	public class OSCManager implements IOSCConnectorListener {
		...
		public function OSCManager(connectorIn:IOSCConnector = null, connectorOut:IOSCConnector = null, autoStart:Boolean = true) {
			...
			this.oscAddressSpace = new OSCAddressSpace();
		...
		public function close():void
		{
			...
			if (_connectorIn) connectorIn.close();
			if (_connectorOut) connectorOut.close();
		...
		{
			if (this._connectorOut && this._connectorOut is UDPConnector)
			{
				(this._connectorOut as UDPConnector).sendOSCPacketTo(oscPacket, host, port);
			}
		}
