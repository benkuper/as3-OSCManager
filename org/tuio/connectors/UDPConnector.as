package org.tuio.connectors
{
	...
	public class UDPConnector implements IOSCConnector
	{
		 ...
		 *
		 */
		public function UDPConnector(host:String = "127.0.0.1", port:int = 3333, bind:Boolean = true,connect:Boolean = true)
		{
			this.listeners = new Array();
			setPort(host, port, bind, connect);
		}
		
		
		public function setPort(host:String, port:int, bind:Boolean = true, connect:Boolean = true):void
		{
			if (connection != null)
			{
				trace(port, connection.localPort, connection.remotePort);
				if (port == connection.localPort) return;
				close();
			}
			
			this.connection = new OSCDatagramSocket(host, port, bind,connect);
			...
			if (packet != null) {
				if (this.listeners.length > 0) {
					...
					for each(var l:IOSCConnectorListener in this.listeners) {
						...
						var cPacket:ByteArray = copyPacket(packet);
						if (OSCBundle.isBundle(packet)) {
							l.acceptOSCPacket(new OSCBundle(packet,e.srcAddress,e.srcPort));
						} else if (OSCMessage.isMessage(packet)) {
							l.acceptOSCPacket(new OSCMessage(packet,e.srcAddress,e.srcPort));
						} else {
		...
		public function sendOSCPacketTo(oscPacket:OSCPacket, host:String, port:int):void
		{
			this.connection.send(oscPacket.getBytes(), 0, 0, host, port);
		}
		...
		public function close():void
		{
			...
			this.connection.removeEventListener(OSCEvent.OSC_DATA, receiveOscData);
		}
	}
}